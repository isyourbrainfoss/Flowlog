    ...
    onUpgrade: (Migrator m, int from, int to) async {
      ...
      if (from < 6) {
        final beanColumns = await m.database
            .customSelect('PRAGMA table_info(beans)', readsFrom: {beans})
      ...
      if (from < 12) {
        final shotColumns = await m.database
            .customSelect('PRAGMA table_info(shots)', readsFrom: {shots})
            .map((row) => row.read<String>('name'))
            .get();
      ...
      if (from < 19) {
        final beanColumns = await m.database
            ...
            .map((row) => row.read<String>('name'))
            .get();
        if (!beanColumns.contains('empty')) {
          await m.database.customStatement(
            'ALTER TABLE beans ADD COLUMN empty INTEGER NOT NULL DEFAULT 0',
          );
        }
      }
    },
